    ...
    d3v3.csv('chart6/absolute_geburtenzahlen.csv', function(error, data) {
    ...
    var chart = makeLineChart(data, 'Monat', {
        '2000': {column: 'y2000'},
        '2001': {column: 'y2001'},
        '2002': {column: 'y2002'},
        '2003': {column: 'y2003'},
...
//Formatter functions for the axes
    chartObj.formatAsNumber = d3v3.format(".0f");
    chartObj.formatAsDecimal = d3v3.format(".2f");
    chartObj.formatAsCurrency = d3v3.format("$.2f");
    ...
    chartObj.bisectYear = d3v3.bisector(chartObj.xFunct).left; //< Can be overridden in definition

//Create scale functions
    chartObj.xScale = d3v3.scale.linear().range([0, chartObj.width]).domain(d3v3.extent(chartObj.data, chartObj.xFunct)); //< Can be overridden in definition
...
//Create axis
    chartObj.xAxis = d3v3.svg.axis().scale(chartObj.xScale).orient("bottom").tickFormat(chartObj.xFormatter); //< Can be overridden in definition

    chartObj.yAxis = d3v3.svg.axis().scale(chartObj.yScale).orient("left").tickFormat(chartObj.yFormatter); //< Can be overridden in definition
...
// Build line building functions
    ...
    for (var yObj in yObjs) {
        yObjs[yObj].line = d3v3.svg.line().interpolate("cardinal").x(function (d) {
...
// Change chart size according to window size
    chartObj.update_svg_size = function () {
        ...
        d3v3.selectAll(".focus.line").attr("y2", chartObj.height);
    ...
    chartObj.bind = function (selector) {
        chartObj.mainDiv = d3v3.select(selector);
        ...
        chartObj.chartDiv = d3v3.select(chartSelector);
        d3v3.select(window).on('resize.' + chartSelector, chartObj.update_svg_size);
...
// Render the chart
    chartObj.render = function () {
        ...
        // month label
        focus.append("text").attr("class", "focus month").attr("x", 9).attr("y", 7);
        // Focus line
        ...
        return chartObj;
        function mousemove() {
            var x0 = chartObj.xScale.invert(d3v3.mouse(this)[0]), i = chartObj.bisectYear(dataset, x0, 1), d0 = chartObj.data[i - 1], d1 = chartObj.data[i];
            try {
